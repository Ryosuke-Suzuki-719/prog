<!-- EZOCAIDでお客様情報を検索するページ、全店、台帳を閲覧できる機能を足す必要がある -->
<?php
session_start();
// 関数ファイル読み込み
...
$ezoca_search = $_POST["ezoca_search"];

$pdo = connect_to_db();

// SQLを実行
$sql = "SELECT * FROM customer_table WHERE ezoca = '$ezoca_search'";
...
if ($status == false) {
  ...
  // 個別の化粧品台帳に情報を登録
        ...
        $output .="<td><a href='daicho_read3.php?name={$record["name"]}&ezoca={$record["ezoca"]}'>購入情報を表示する</a></td>";
        $output .= "</tr>";
      }
      unset($record);
    }
  ...
  <head>
    ...
    <title>全店舗からのお客様のEZOCAIDからの検索結果一覧</title>
  </head>
  <body>
  <!-- 検索結果を表示 -->
    <fieldset>
      <legend>全店舗からのお客様のEZOCAIDからの検索結果一覧</legend>
      <table border="1">
        <thead>
          <tr>
