<!-- 化粧品台帳一覧、全店、該当者一覧表示ページ、編集、削除できない -->
<?php
// セッションの開始
session_start();
// check_session_id();
...
$sql = "SELECT * FROM daicho_table WHERE ezoca=:ezoca AND name =:name ORDER BY deadline DESC";
...
// 個別の化粧品台帳に情報を登録
      $output .="<td colspan='4'><a href='daicho_input.php?name={$record["name"]}&ezoca={$record["ezoca"]}'>化粧品台帳に登録する</a></td>";
      $output .= "</tr>";      
    }
    unset($record);
  }
